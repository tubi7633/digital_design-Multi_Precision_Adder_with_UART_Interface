...
//hybrid_carry_select_lookahead_adder #(
//  .WIDTH (ADDER_WIDTH),   // match the width of your "slice" (32)
//  .BLOCK (16)              // e.g. 8-bit CLA blocks inside the 32-bit slice
//) hybrid_inst (
//  .iA    (operandA),
//  .iB    (operandB),
//  .iCin  (carry_in),
...
//);
   ...
   // Replace this un-guarded code:
//
//    always @(posedge iClk) begin
//      if (iRst)
//        regResult <= {OPERAND_WIDTH{1'b0}};
//      else
//        regResult <= { result, regResult[OPERAND_WIDTH-1:ADDER_WIDTH] };
//    end
//
//    always @(posedge iClk) begin
//      if (iRst)
...
always @(posedge iClk) begin
  ...
  else if (rFSM_current == s_ADD_FIRST ||
           rFSM_current == s_ADD_WORDS) begin
    // only shift in a new slice during the two ADD states
    regResult <= { result, regResult[OPERAND_WIDTH-1:ADDER_WIDTH] };
  end
  // else keep the old value-no extra shifts!
end
...
always @(posedge iClk) begin
  if (iRst) begin
    regCout <= 1'b0;
           ...
           rFSM_current == s_ADD_WORDS) begin
    // only latch carry_out during the ADD states
    regCout <= carry_out;
  end
  // else hold it
end
    ...
    assign muxCarryIn = (muxCarry_sel == 0) ? iOp : regCout;
...
//    assign carry_in = (iOp == 1)? 1: muxCarryIn;
